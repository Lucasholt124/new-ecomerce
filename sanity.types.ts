...
// Query: *[  _type == "customer"  && email == $email][0]{  _id,  email,  name,  clerkUserId,  asaasCustomerId, // Atualizado de stripeCustomerId  createdAt}
...
// Variable: CUSTOMER_BY_ASAAS_ID_QUERY
// Query: *[  _type == "customer"  && asaasCustomerId == $asaasCustomerId][0]{  _id,  email,  name,  clerkUserId,  asaasCustomerId, // Atualizado de stripeCustomerId  createdAt}
export type CUSTOMER_BY_ASAAS_ID_QUERYResult = {
  _id: string;
  email: string | null;
  name: string | null;
  clerkUserId: string | null;
...
// Query: *[_type == "order" && clerkUserId == $clerkUserId] | order(createdAt desc) {    _id,    orderNumber,    total,    status,    createdAt,    "itemNames": items[].product->name,    "itemImages": items[].product->images[0].asset->url,    "itemCount": count(items)  }
...
// Variable: ORDER_BY_ASAAS_PAYMENT_ID_QUERY
// Query: *[_type == "order" && asaasPaymentId == $asaasPaymentId][0] {    _id,    orderNumber,    status,    // Importante trazer os itens para podermos baixar o estoque    items[]{      quantity,      product    }  }
export type ORDER_BY_ASAAS_PAYMENT_ID_QUERYResult = {
  _id: string;
  orderNumber: string | null;
  status: "cancelled" | "delivered" | "paid" | "pending" | "shipped" | null;
  items: Array<{
...
// Variable: STATS_ORDER_COUNT_QUERY
// Query: count(*[_type == "order"])
export type STATS_ORDER_COUNT_QUERYResult = number;
// Variable: STATS_TOTAL_REVENUE_QUERY
// Query: math::sum(*[  _type == "order"  && status in ["paid", "shipped", "delivered"]].total)
export type STATS_TOTAL_REVENUE_QUERYResult = number;
// Variable: STATS_ORDERS_LAST_7_DAYS_QUERY
// Query: *[  _type == "order"  && createdAt >= $startDate  && !(_id in path("drafts.**"))] | order(createdAt desc) {  _id,  orderNumber,  total,  status,  createdAt,  "itemCount": count(items),  items[]{    quantity,    priceAtPurchase,    "productName": product->name,    "productId": product->_id  }}
export type STATS_ORDERS_LAST_7_DAYS_QUERYResult = Array<{
  _id: string;
  orderNumber: string | null;
  total: number | null;
  status: "cancelled" | "delivered" | "paid" | "pending" | "shipped" | null;
...
// Variable: STATS_ORDER_STATUS_DISTRIBUTION_QUERY
// Query: {  "paid": count(*[_type == "order" && status == "paid" && !(_id in path("drafts.**"))]),  "shipped": count(*[_type == "order" && status == "shipped" && !(_id in path("drafts.**"))]),  "delivered": count(*[_type == "order" && status == "delivered" && !(_id in path("drafts.**"))]),  "cancelled": count(*[_type == "order" && status == "cancelled" && !(_id in path("drafts.**"))])}
export type STATS_ORDER_STATUS_DISTRIBUTION_QUERYResult = {
  paid: number;
  shipped: number;
  delivered: number;
  cancelled: number;
...
declare module "@sanity/client" {
  interface SanityQueries {
    ...
    "*[\n  _type == \"customer\"\n  && asaasCustomerId == $asaasCustomerId\n][0]{\n  _id,\n  email,\n  name,\n  clerkUserId,\n  asaasCustomerId, // Atualizado de stripeCustomerId\n  createdAt\n}": CUSTOMER_BY_ASAAS_ID_QUERYResult;
    ...
    "\n  *[_type == \"order\" && asaasPaymentId == $asaasPaymentId][0] {\n    _id,\n    orderNumber,\n    status,\n    // Importante trazer os itens para podermos baixar o estoque\n    items[]{\n      quantity,\n      product\n    }\n  }\n": ORDER_BY_ASAAS_PAYMENT_ID_QUERYResult;
    "count(*[_type == \"order\"])": ORDER_COUNT_QUERYResult | STATS_ORDER_COUNT_QUERYResult;
    "math::sum(*[\n  _type == \"order\"\n  && status in [\"paid\", \"shipped\", \"delivered\"]\n].total)": TOTAL_REVENUE_QUERYResult | STATS_TOTAL_REVENUE_QUERYResult;
    "*[\n  _type == \"order\"\n  && createdAt >= $startDate\n  && !(_id in path(\"drafts.**\"))\n] | order(createdAt desc) {\n  _id,\n  orderNumber,\n  total,\n  status,\n  createdAt,\n  \"itemCount\": count(items),\n  items[]{\n    quantity,\n    priceAtPurchase,\n    \"productName\": product->name,\n    \"productId\": product->_id\n  }\n}": ORDERS_LAST_7_DAYS_QUERYResult | STATS_ORDERS_LAST_7_DAYS_QUERYResult;
    "{\n  \"paid\": count(*[_type == \"order\" && status == \"paid\" && !(_id in path(\"drafts.**\"))]),\n  \"shipped\": count(*[_type == \"order\" && status == \"shipped\" && !(_id in path(\"drafts.**\"))]),\n  \"delivered\": count(*[_type == \"order\" && status == \"delivered\" && !(_id in path(\"drafts.**\"))]),\n  \"cancelled\": count(*[_type == \"order\" && status == \"cancelled\" && !(_id in path(\"drafts.**\"))])\n}": ORDER_STATUS_DISTRIBUTION_QUERYResult | STATS_ORDER_STATUS_DISTRIBUTION_QUERYResult;
