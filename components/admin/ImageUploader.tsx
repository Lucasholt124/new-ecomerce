  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        // Upload do asset para o Sanity
        const asset = await client.assets.upload("image", file, {
          filename: file.name,
        });

        // Cria o objeto de imagem com uma chave única
        newImages.push({
          _type: "image",
      ...
      setUploadProgress("Falha ao enviar. Tente novamente.");
    ...
    <div className="space-y-4">
      ...
      {currentImages.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-zinc-200 py-8 dark:border-zinc-700">
          ...
          <p className="text-xs text-zinc-400 dark:text-zinc-500">
            Clique em enviar para adicionar fotos
          </p>
        </div>
      )}
      ...
      {currentImages.length > 0 && (
        <p className="text-xs text-zinc-500 dark:text-zinc-400">
          A primeira imagem será a principal. Use as setas para reordenar.
        </p>
      )}
    </div>
  );
...
function ImageThumbnail({
  ...
  // Constrói a URL a partir da referência do asset
