  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex gap-3">
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            >
              {isComplete
                ? `${displayName} concluído` // Traduzido
                : `${displayName}...`}
            </span>
            {searchQuery && (
              <span className="text-xs text-zinc-500 dark:text-zinc-400">
                Busca: &quot;{searchQuery}&quot; {/* Traduzido */}
              </span>
            )}
            {orderStatus && (
              <span className="text-xs text-zinc-500 dark:text-zinc-400">
                Filtro: {orderStatus} {/* Traduzido */}
              </span>
            )}
          </div>
        </div>
      ...
      {hasProducts && productResult?.products && (
        <div className="ml-11 mt-2">
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-2">
            {productResult.products.length} produto{productResult.products.length !== 1 ? "s" : ""} encontrado{productResult.products.length !== 1 ? "s" : ""} {/* Traduzido */}
          </p>
          <div className="space-y-2">
      ...
      {hasOrders && orderResult?.orders && (
        <div className="ml-11 mt-2">
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-2">
            {orderResult.orders.length} pedido{orderResult.orders.length !== 1 ? "s" : ""} encontrado{orderResult.orders.length !== 1 ? "s" : ""} {/* Traduzido */}
          </p>
          <div className="space-y-2">
