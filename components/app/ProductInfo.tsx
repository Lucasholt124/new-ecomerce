...
interface Product {
  ...
  stock: number | null;
  // A tipagem de imagens deve permitir chaves extras e nulos
  images?: Array<{
    _key?: string;
    asset?: {
      url: string | null;
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="mt-6 flex flex-col gap-3">
        ...
        />
        <AskAISimilarButton productName={product.name ?? "este produto"} />
      </div>
      ...
      <div className="mt-6 space-y-2 border-t border-zinc-200 pt-6 dark:border-zinc-800">
        ...
        {product.color && (
          <div className="flex justify-between text-sm">
            <span className="text-zinc-500 dark:text-zinc-400">Cor</span>
            <span className="font-medium capitalize text-zinc-900 dark:text-zinc-100">
              {product.color}
            </span>
          </div>
        ...
        {product.assemblyRequired !== null &&
          product.assemblyRequired !== undefined && (
            <div className="flex justify-between text-sm">
              <span className="text-zinc-500 dark:text-zinc-400">Montagem</span>
              <span className="font-medium text-zinc-900 dark:text-zinc-100">
                {product.assemblyRequired ? "Necessária" : "Não necessária"}
              </span>
            </div>
          )}
      </div>
